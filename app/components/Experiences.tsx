...
const Experience: React.FC<ExperienceProps> = ({ language = "TR" }) => {
  ...
  // Function to get border radius based on item title or type
  const getBorderRadius = (item: ExperienceItem) => {
    const title = item.title.toLowerCase();

    if (
      title.includes("ingilizce hazırlık") ||
      title.includes("english preparation")
    ) {
      return "rounded-t-4xl";
    } else if (title.includes("lise") || title.includes("high school")) {
      return "rounded-b-4xl";
    } else if (
      title.includes("elektrik elektronik") ||
      title.includes("electrical electronics")
    ) {
      return "rounded-t-4xl";
    }
        ...
        >
          ...
          <div className="space-y-6">
            {experienceData[
              ...
              const borderRadius = getBorderRadius(item);
              ...
              return (
                ...
                >
                  <div
                    className={`bg-info ${borderRadius} transition-all duration-300 overflow-hidden`}
                  >
                    {/* Main Content */}
                    <div className="p-6">
                      <div className="flex items-start space-x-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {/* Expand Button - Only show if item has achievements */}
                          {showExpandButton && (
                            <button
